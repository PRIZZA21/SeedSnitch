...
const IncubatorTable = () => {
    ...
    const blog_detailsHandler = (id) => { navigate(`/admin/blogs/${id}`)}
            ...
            <table className="min-w-full leading-normal">
                ...
                <thead>
                    {table_headers.map((theader)=>(
                        <th className="px-5 py-3 text-center border-b-2 border-gray-200 bg-gray-100 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider">
                            {theader}
                        </th>
                    ))}
                </thead>
                ...
                <tbody>
                    { blogsList && blogsList.map((blog)=>(
                        <tr>
                            ...
                            <TableData child={<img src={`/${blog.banner}`} className="w-8 h-8" alt=''/>} />
